...
#ifdef EGE_USE_GLFW3
ege3dGraphicContext::ege3dGraphicContext(std::string caption, int width, int height, int windowPositionLeft, int windowPositionTop, GLFWwindow* share){
    EGEFUNCTION
    ...
    if (!glfwInit()) {
        ...
        std::cout << " GLFW Version : " << glfwVer << "\n" << std::endl;
        // glfwSetErrorCallback(GLFW_error_callback);
    
        #ifndef EGE_WIN64
            // NOTE: 2020 MAR 31 -  If version setup doesn't match Opengl actual version,
            ...
            glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, 3); egeLog(EGE_CHECK_FOR_OPENGL_ERROR,"glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, 1);");
        #else    
    ...
    glfwSetWindowPos(window, windowPositionLeft, windowPositionTop); lastError__=egeLog(EGE_CHECK_FOR_OPENGL_ERROR, " error rises calling : glfwSetWindowPos(window, 0, 0)");
    ...
    glCullFace(GL_FRONT_AND_BACK);egeLog(EGE_CHECK_FOR_OPENGL_ERROR,"glCullFace(GL_BACK);");  
...
void ege3dGraphicContext::OpenGLES2Initialize(void){
    ...
    glGetShaderInfoLog(	fs, 2048, &len1, infoLog1);
    ...
    glGetShaderInfoLog(	vs, 2048, &len2, infoLog2);
    ...
    GLuint attribLocation_VERTS = glGetAttribLocation(shaderProgram, "a_position");
    GLuint attribLocation_TEXCOORDS = glGetAttribLocation(shaderProgram, "a_texCoord");
    ...
    glBindBuffer(GL_ARRAY_BUFFER, vbo);
    glVertexAttribPointer(attribLocation_VERTS, 2, GL_FLOAT, GL_FALSE, 20, (void*)0);
    glEnableVertexAttribArray(attribLocation_VERTS);
    glVertexAttribPointer(attribLocation_TEXCOORDS, 2, GL_FLOAT, GL_FALSE, 20, (void*)8);
    glEnableVertexAttribArray(attribLocation_TEXCOORDS);
    glBindVertexArray(0); // don't forget to unbind current VAO
    
    // Get the sampler location
    textureUniform = glGetUniformLocation(shaderProgram, "s_texture");
    glUniform1i(textureUniform, 0); // Texture unit 0 is for base images.
